      ...
      <div class="bill-inner">
        ...
        <template v-if="billStep === 'items'">
          ...
          <div v-if="hasActivePkg" class="section" style="margin-top: 6px">
            ...
            <div v-if="multiPkgWarning" class="tiny" style="color: var(--warning); padding: 2px">
...
function pkgInUse(p) {
  return billSel.value.some((t) => txCovered(t) && pkgFor(t)?.id === p.id)
}
...
const needsOopReason = computed(
  () => hasActivePkg.value && billSel.value.some((t) => !txInPackage(t)),
)
...
async function createInvoice(usePackage) {
  ...
  try {
    ...
    if (usePackage) {
      ...
      const covered = billSel.value.find((t) => txCovered(t))
